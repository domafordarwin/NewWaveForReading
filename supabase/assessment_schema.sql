...
CREATE INDEX IF NOT EXISTS idx_student_parent_code ON public.student_parent_relations(student_code);

-- student_id 기반으로 student_code 자동 채움
create or replace function public.fill_student_code_for_relation()
returns trigger
language plpgsql
as $$
begin
  if new.student_code is null or new.student_code = '' then
    select u.student_code into new.student_code
    from public.users u
...
drop trigger if exists trg_student_parent_code on public.student_parent_relations;
create trigger trg_student_parent_code
before insert or update of student_id, student_code
on public.student_parent_relations
for each row
execute function public.fill_student_code_for_relation();
...
DROP POLICY IF EXISTS "Allow all for student_answers" ON public.student_answers;
DROP POLICY IF EXISTS "Allow all for ai_evaluations" ON public.ai_evaluations;
DROP POLICY IF EXISTS "Allow all for teacher_feedbacks" ON public.teacher_feedbacks;
DROP POLICY IF EXISTS "Allow all for student_parent_relations" ON public.student_parent_relations;
DROP POLICY IF EXISTS "Allow all for classes" ON public.classes;
DROP POLICY IF EXISTS "Allow all for class_students" ON public.class_students;
